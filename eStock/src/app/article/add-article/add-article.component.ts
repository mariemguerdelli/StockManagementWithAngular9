...
export class AddArticleComponent implements OnInit {
  ...
  infoForm() {
    this.crudApi.dataForm = this.fb.group({
        ...
        stk_init: [0, [Validators.required]],
        code_cat: ['', [Validators.required]],
        code_scat: ['', [Validators.required]],
...
onSelectScateg(id_scat: string)
{
 this.scategorieService.getData(id_scat).subscribe(
    response =>{
      this.scategorie = response;
      this.wcode = (10000 + this.scategorie.rang).toString().substring(1);
      this.wcode = this.scategorie.id_cat+this.scategorie.code+this.wcode;
